...
image_folder = "/home/athip/psu/3/ecosys/proj/dev/model/data/val/images"  # Replace with the path to your image folder
...
# Load your model
model_data = torch.load("/home/athip/psu/3/ecosys/proj/runs/detect/train11/weights/best.pt")
model = model_data['model'].to(device=0, dtype=torch.float32)
test_input = torch.rand([1,3,size,size]).to(device=0,dtype=torch.float32)
images_score = []
images_bbox = []
# Time the inference
with torch.no_grad():
    ...
    for inputs in data_loader:  # Iterate over the images in the folder
        # st = time.time()
        inputs = inputs.to(device=0, dtype=torch.float32)  # Move the input to the device (GPU)
        output = model(inputs)
        for out in output[0].to(device=0):
            filtered_boxes, filtered_scores = nms_function(output=out.to(device = 0))
